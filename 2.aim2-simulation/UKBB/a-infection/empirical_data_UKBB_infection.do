...
logistic covid_test_phase1 sd_bmi i.eduyears_quali i.sex sd_age i.current_smoke sd_tdi, coef
matrix results = r(table)
...
local x=4
foreach var in covid_positive_phase1 {

	local x=`x'+1	
	putexcel set 20210309_UKBB_simulations_infection, sheet(prevalence_and_distributions) modify
	
	tab `var' if mass_test==0, matcell(numbers)
